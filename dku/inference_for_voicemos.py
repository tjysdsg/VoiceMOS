...
def inference(mode, model, ep, dataloader, systems, save_dir, name, dataset_name, return_posterior_scores=False):
    ...
    for i, batch in enumerate(tqdm(dataloader)):
        ...
        with torch.no_grad():
            if mode == "mean_net":
                pred_mean_scores = model.only_mean_inference(spectrum=mag_sgrams_padded, spk_embed=spk_embed)
            elif mode == "all_listeners":
                pred_mean_scores, posterior_scores = model.average_inference(
                    spectrum=mag_sgrams_padded, include_meanspk=return_posterior_scores, spk_embed=spk_embed
                )
                posterior_scores = posterior_scores.cpu().detach().numpy()
                post_scores.extend(posterior_scores.tolist())
            elif mode == "mean_listener":
                pred_mean_scores = model.mean_listener_inference(
                    spectrum=mag_sgrams_padded, spk_embed=spk_embed
                )
            else:
...
def main():
    ...
    dataset = get_dataset(args.dataset_name, args.data_dir, args.spk_embed_dir, args.phase)
