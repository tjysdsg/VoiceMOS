...
class MBNet(nn.Module):
    def __init__(self, config):
        ...
        if config["use_mean_net"]:
            ...
            self.mean_net_dnn = Projection(config["mean_net_rnn_dim"] * 2, config["mean_net_dnn_dim"],
                                           activation, config["mean_net_output_type"],
                                           config["mean_net_range_clipping"])
        ...
        decoder_conv_output_dim = self._get_decoder_conv_output_dim(config["audio_input_dim"], len(
            config["decoder_conv_chs"]) + 1)  # +1: encoder conv
    ...
    def forward(self, spectrum, judge_id):
        ...
        encoder_outputs = torch.cat([encoder_outputs, judge_feat],
                                    dim=1)  # concat along channel dimension, resulting in shape [batch, ch, t, d]
    ...
    def average_inference_v1(self, spectrum, include_meanspk=False):
        ...
        if self.config["encoder_type"] == "simple":
            ...
            spectrum = spectrum.unsqueeze(1)
            encoder_outputs = self.encoder(spectrum)
            encoder_outputs = torch.stack([encoder_outputs for i in range(actual_num_judges)],
                                          dim=1)  # (bs, nj, ch, time, feat_dim)
